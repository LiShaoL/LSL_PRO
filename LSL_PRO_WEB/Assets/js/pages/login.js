/*lsl 2016.11.23*/
$(function () {
    var username = LSL_login.getCookie("username");
    var userpass = LSL_login.getCookie("userpass");
...
var LSL_login = {
    passKey: "4c05c54d952b11e691d76c0b843ew7f9",
    //save cookie
    setCookie: function (cname, cvalue, a) {
        var d = new Date();
        d.setTime(d.getTime() + (a * 24 * 60 * 60 * 1000));
        var expires = "expires=" + d.toUTCString();
        document.cookie = cname + "=" + encrypt(escape(cvalue), this.passKey) + "; " + expires;
    },
    //get cookie
    ...
    saveUserInfo: function () {
        if ($("#RememberMe").is(":checked")) {
            this.setCookie("username", $("#username").val().trim(), 7)
            this.setCookie("userpass", $("#userpass").val().trim(), 7)
        }
    },
    callBack: function (data) {
        if (data.code == "-1") {
            ...
            window.open("/Index/Index", "_self");
        }
},
    //check login  
    CheckLogin: function () {        
        LSLCommon.jQajax("post", "/Login/LoginP", $("#form_login").serialize(), function (data) { LSL_login.callBack(data);}, false);
    }
}
//加密算法（加密）
